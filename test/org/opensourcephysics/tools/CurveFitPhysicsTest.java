...
public class CurveFitPhysicsTest {
    ...
    static void run() {
        ...
        OSPRuntime.setPreferredDecimalSeparator(".");
        KnownPolynomial motion = new KnownPolynomial(new double[]{-.0098,2.688,-4.989});
        double[] motionErrors = {.023,.013,.0018};
        String[][] derived = CurveFitReport.motionResults(motion,null,motionErrors,true,"s","m","y");
        check(derived.length==2,"quadratic provides velocity at zero and acceleration");
        check(derived[0][0].equals("y velocity at t=0"),"reference time explicit");
        near(Double.parseDouble(derived[0][1]),2.688,"velocity is B");
        near(Double.parseDouble(derived[0][2]),.013,"velocity error is sigma B");
        near(Double.parseDouble(derived[1][1]),-9.978,"acceleration is twice A");
        near(Double.parseDouble(derived[1][2]),.046,"acceleration error is twice sigma A");
        check(derived[0][3].equals("m/s") && derived[1][3].equals("m/s^2"),"motion result units");
        String copied = CurveFitReport.appendMotionResults("",derived);
        check(copied.contains("y acceleration\t-9.978\t0.046\tm/s^2"),"motion values exported once at full precision");
        check(CurveFitReport.motionResults(motion,null,motionErrors,true,"s","m",null).length==0,"no motion labels without host metadata");
        check(CurveFitReport.motionResults(new UserFunction("Arbitrary"),null,null,true,"s","m","y").length==0,"no guessed user-function motion");
        check(CurveFitReport.motionResults(new KnownPolynomial(new double[]{1,2,3,4}),null,null,true,"s","m","y").length==0,"no constant acceleration for cubic");
        derived=CurveFitReport.motionResults(motion,new boolean[]{true,false,false},motionErrors,true,"s","m","y");
        check(derived[1][2].equals("N/A") && derived[1][4].equals("Yes"),"fixed acceleration has no estimated error");
        derived=CurveFitReport.motionResults(motion,null,motionErrors,false,"s","m","y");
        check(derived[0][2].equals("N/A") && derived[1][2].equals("N/A"),"manual motion does not reuse fitted errors");
        derived=CurveFitReport.motionResults(new KnownPolynomial(new double[]{7,-2}),null,new double[]{.03,.2},true,"s","m","x");
        check(derived.length==1 && derived[0][0].equals("x velocity"),"line has velocity only");
        near(Double.parseDouble(derived[0][1]),-2,"line velocity sign and ordering");
        near(Double.parseDouble(derived[0][2]),.03,"line velocity uncertainty");
        near(motion.getParameterValue(0),-4.989,"derivation preserves coefficients");
