...
/**
 * ClassicalApp calculates and displays the classical trajectory of particle near a spherically symmetric
 * gravitational mass by extending AbstractAnimation and implementing the doStep method.
 *
 * @author W. Christian
 * @version 1.0
 */
public class ClassicalApp extends AbstractSimulation implements InteractiveMouseHandler, PropertyChangeListener {
   ...
   protected void doStep() {
     if(inspector!=null && inspector.newInspectorData) {
      ...
      Iterator it = trajectories.iterator();
      while(it.hasNext()) {
         ((ClassicalTrajectory) it.next()).stepTime();
      }
      time += dt;
      ...
      if (time >= maxTime) {
        ...
        stopSimulation();
        plottingPanel.setMessage("Simulation done.", PlottingPanel.TOP_LEFT);
      }
   }
...
public void initialize() {
   ...
   public void handleMouseAction(InteractivePanel panel, MouseEvent evt) {
      ...
      if(ia==null) return;
      if((ia instanceof ClassicalTrajectory)
          &&((panel.getMouseAction()==InteractivePanel.MOUSE_CLICKED)
      ...
      if((ia==defaultTrajectory)&&(inspector!=null)&&(panel.getMouseAction()==InteractivePanel.MOUSE_DRAGGED)) {
            inspector.updateDataAndScale(defaultTrajectory);
      }
   }
...
class ClassicalLoader implements XML.ObjectLoader {
   ...
   public Object loadObject(XMLControl control, Object obj) {
      ...
      app.markers = (ArrayList) control.getObject("markers");
