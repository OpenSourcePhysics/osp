...
/**
 * ShellParticle models a particle attached to a shell near a spherically
 * symmetric gravitational field using Schwarzschild coordinates.
 *
...
public class ShellParticle extends AbstractTrajectory {
	...
	/**
	 * Initializes the particle's state using the given state array. The state array
	 * = {t, tau}
	 *
	...
	public void setXY(double x, double y) {
		...
		for (int i = 0, n = cluster.size(); i < n; i++) {
			AbstractTrajectory t = cluster.get(i);
			if (t != this) {
				t.setX(t.getX() + dx);
				t.setY(t.getY() + dy);
			}
	...
	/**
	 * Gets the energy per unit mass. E is computed during initialization.
	 * 
	 * @return double
	 */
	public double getE() {
