...
public class EigenstateWellSuperposition implements QMSuperposition{
  ...
  public EigenstateWellSuperposition(int numpts, double xmin, double xmax) {
...
public boolean setCoef(double[] re, double[] im) {
    ...
    for(int i=0; i<n; i++){
      double[] stateArray = eigenstates[i];
      double kx=0, delta=Math.PI*(i+1)/(x.length-1);
      for(int j=0; j<numpts; j++){
        stateArray[j]=Math.sqrt(2.0/L)*Math.sin(kx);
        kx+=delta;  // increment the phase by delta
      }
    }
    return true;
  }
...
public double getEigenValue(int i) {
     return (i+1)*(i+1)*PISQR*energyScale/L/L;
  }
...
public void update(double time){
    ...
    double kk=-PISQR*energyScale*time/L/L;
    for(int i=0, ns=recoef.length; i<ns; i++){
      double re=recoef[i];
      double im=imcoef[i];
      double phase=kk*(i+1)*(i+1);
      double sin=Math.sin(phase);
      double cos=Math.cos(phase);
      double[] stateArray = eigenstates[i];
      for(int j=0, n=x.length; j<n; j++){
        rePsi[j]+=(re*cos-im*sin)*stateArray[j];
        imPsi[j]+=(im*cos+re*sin)*stateArray[j];
      }
    }
  }
