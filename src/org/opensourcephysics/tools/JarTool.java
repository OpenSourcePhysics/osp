...
//  }
	...
	private static Map<String, Map<String, ZipEntry>> jarContents = new HashMap<String, Map<String, ZipEntry>>(); // added
																													// by
																													// D
																													// Brown
																													// 2007-10-31
	...
	@Override
	public void run() {
		compressList(instanceSources, instanceParent, instanceTarget, instanceManifest, instancePolicy,
				instanceOwnerFrame);
	}

	private JarTool(ArrayList<String> aSources, File aParent, File aTarget, Manifest aManifest, OverwriteValue aPolicy,
			Frame _anOwner) {
		this();
	...
	/**
	 * Sets the overwrite feature to false. By default, the packaging methods
	 * create() and append() will warn the user before overwritting a duplicated
	 * file. Calling this method before running those methods disables this warning
	 * and makes sure a new file will NEVER overwrite an older one. The methods
	 * create() and append() set the warning feature back.
	 * 
	 * @see alwayOverwrite
	 */
	...
	/**
	 * Sets the overwrite feature to true. By default, the packaging methods
	 * create() and append() will warn the user before overwritting a duplicated
	 * file. Calling this method before running those methods disables this warning
	 * AND makes sure a new file will ALWAYS overwrite an older one. The methods
	 * create() and append() set the warning feature back.
	 * 
	 * @see neverOverwrite
	 */
	 ...
	 * 
	 * Sets the owner frame for progress dialogs that may appear when creating a JAR
	 * file.
	 * 
	 * @param owner Frame
	 */
	 ...
	 * called by LaunchBuilder
	 * 
	 * Creates a JAR file out of the list of contents provided. Each entry in the
	 * sources list can be either a single file, a directory, or a compressed (ZIP,
	 * JAR or TRZ) file. Regular files and directories must exist under the given
	 * parent directory, and are saved with the same relative path as provided. As
	 * an exception, you can also add files in parent directories, such as,
	 * "../../../WhateverDir/WhateverFile.xml", but ALL leading "../" will be
	 * removed when saved. Directories are added recursively. Compressed files can,
	 * on the contrary, be placed anywhere and their contents are stored with the
	 * same directory structure as in the original jar file, irrespective of the
	 * original location of the source compressed file. If a file appears more than
	 * once, succesive files could overwrite previous ones. The user will be warned
	 * of this, except if either neverOverwrite() or alwaysOverwrite() have been
	 * invoked immediately before calling this method. (Calling this method resets
	 * the warning feature back.)
	 *
	 * @param sources  ArrayList The list of content files to add. Each item in the
	 *                 list is a String with the relative name of a file or
	 *                 directory under the given parent directory, or of a
	 *                 compressed file anywhere in the hard disk.
	 * @param parent   File The parent directory for all relative filenames
	 * @param target   File The target compressed file. Its name must end in .zip,
	 *                 .jar or .trz. The user will be prompted to confirm the
	 *                 target.
	 * @param manifest Manifest A manifest for the newly created JAR file.
	 * @return File The file that will eventually be created. Note that the main
	 *         work is done using a separate thread, hence the method returns BEFORE
	 *         the JAR file is actually created.
	...
	public File create(ArrayList<String> sources, File parent, File target, Manifest manifest) {
		...
		try {
			...
			if (org.opensourcephysics.controls.XML.forwardSlash(target.getAbsolutePath())
					.equals(OSPRuntime.getLaunchJarPath())) {
				String[] message = new String[] { res.getString("JarTool.JarNotCreated"), //$NON-NLS-1$
						res.getString("JarTool.FileIsForbidden") + " " + target }; //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (warnBeforeOverwritting && target.exists()) {
				int selected = JOptionPane.showConfirmDialog(null,
						DisplayRes.getString("DrawingFrame.ReplaceExisting_message") + " " + //$NON-NLS-1$ //$NON-NLS-2$
...
//  
	/**
	 * Extracts a given file from a compressed (ZIP, JAR or TRZ) file Extensive
	 * changes by D Brown 2007-10-31
	 * 
	 * @param source   File The compressed file to extract the file from
	 * @param filename String The path of the file to extract
...
//  }
	...
	/**
	 * Compresses a directory into a single JAR or ZIP file. If the target file
	 ...
	 * All files/dirs must be in the same directory. If the target file exists it
	 * will be overwritten.
	 * 
	...
	/**
	 * Uncompresses a ZIP or JAR file into a given directory. Duplicated files will
	 * be overwritten.
	 * 
	...
	/**
	 * Uncompresses a ZIP or JAR file into a given directory. The system will issue
	 * a warning before duplicating existing files.
	 * 
	 * @param source          File The compressed file to uncompress
	 * @param targetDirectory File The target directory
	 * @return boolean
	...
	/**
	 * Uncompresses a ZIP or JAR file into a given directory. policy.value indicates
	 * what to do on duplicated files.
	 * 
	 * @param source          File The compressed file to uncompress
	 * @param targetDirectory File The target directory
...
//                  case CANCEL : return false;
	 ...
	 * @param prefix          A prefix to add to the extracted file in order to
	 *                        create the message
	 * @return java.util.List<File> the set of files extracted, null if cancelled
	 */
	static public java.util.List<File> unzipWithWarning(InputStream zipStream, File targetDirectory,
			javax.swing.JLabel label, String prefix) {
		try {
			...
			while ((zipEntry = input.getNextEntry()) != null) {
				if (zipEntry.isDirectory())
					continue;
				if (label != null)
					label.setText(prefix + zipEntry.getName());
				File newFile = new File(targetDirectory, zipEntry.getName());
				if (newFile.exists()) {
					switch (policy.value) {
					case NO_TO_ALL:
	...
	/**
	 * Compresses a list of sources to a final compressed file. policy.value
	 * indicates what to do on duplicated files.
	 * 
	 * @param sources  ArrayList A list of relative filenames
	 * @param parent   File The parent directory to which relative filenames are
	 *                 given
	 ...
	 * @return File
	 */
	static private File compressList(ArrayList<String> sources, File parent, File target, Manifest manifest,
			OverwriteValue policy, Frame owner) {
		// Create a temporary directory
		File temporaryDirectory = null;
		try {
			temporaryDirectory = File.createTempFile("JarTool", ".tmp", target.getParentFile()); // Get //$NON-NLS-1$ //$NON-NLS-2$
																									// a unique name for
																									// our temporary
																									// directory
		...
		if ((temporaryDirectory == null) || !temporaryDirectory.mkdirs()) { // and re-create it as a directory
			String[] message = new String[] { res.getString("JarTool.JarNotCreated"), //$NON-NLS-1$
					res.getString("JarTool.CantCreateTemp") }; //$NON-NLS-1$
			JOptionPane.showMessageDialog((JFrame) null, message, res.getString("JarTool.Error"), //$NON-NLS-1$
					JOptionPane.WARNING_MESSAGE);
			return null;
		}
		...
		for (Iterator<String> it = sources.iterator(); it.hasNext();) { // Copy or uncompress all sources to the
																		// temporary directory
		...
		// Now pack everything into a single compressed file
			...
			if (compress(temporaryDirectory, target, manifest)) {
				...
				String[] message = new String[] { res.getString("JarTool.JarNotCreated"), //$NON-NLS-1$
						res.getString("JarTool.CantCompress") + " " + target.getAbsolutePath() }; //$NON-NLS-1$ //$NON-NLS-2$
				JOptionPane.showMessageDialog((JFrame) null, message, res.getString("JarTool.Error"), //$NON-NLS-1$
						JOptionPane.WARNING_MESSAGE);
			}
		}
	...
	static private StringBuffer processFile(String filename, File file, File targetDirectory, OverwriteValue policy) {
		...
		if (file.isDirectory()) {
			...
			for (int i = 0, n = filesInDir.length; i < n; i++) {
				errorMessage.append(
						processFile(filename + "/" + filesInDir[i].getName(), filesInDir[i], targetDirectory, policy)); //$NON-NLS-1$
			}
			return errorMessage;
		}
		...
		if (filenameLowerCase.endsWith(".jar") || filenameLowerCase.endsWith(".zip") //$NON-NLS-1$ //$NON-NLS-2$
				|| filenameLowerCase.endsWith(".trz")) { //$NON-NLS-1$
