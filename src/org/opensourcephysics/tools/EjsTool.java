...
public class EjsTool {
	...
	/**
	 * Runs the Ejs model corresponding to the given class. The model and resources
	 * required will be extracted using the ResourceLoader utility.
	 * 
	 * @param _ejsClass Class
	...
	/**
	 * Runs the Ejs model corresponding to the given class providing a given
	 * password The model and resources required will be extracted using the
	...
	static public boolean runEjs(Class<?> _ejsClass, String _password) {
		try {
			...
			String[] message = new String[] { res.getString("EjsTool.EjsNotRunning"), //$NON-NLS-1$
					res.getString("EjsTool.NoModel") + " " + _ejsClass.getName() }; //$NON-NLS-1$ //$NON-NLS-2$
			JOptionPane.showMessageDialog((JFrame) null, message, res.getString("EjsTool.Error"), //$NON-NLS-1$
	...
	/**
	 * To be used by EJS version 3.47 and earlier only. It is here to make sure that
	 * the information saved matches the information read by runEjs().
	 * 
	...
	/**
	 * To be used by EJS version 4.0 and later only. It is here to make sure that
	 * the information saved matches the information read by runEjs().
	 * 
	 * @param _binDirectoryPath    String The binary directory from which EJS was
	 *                             launched
	...
	/**
	 * Gets the path of a file in standard form. If it is a directory, the path ends
	 * in "/"
	 */
...
//                    for (int i=0; i<cmdarray.length; i++) System.out.println ("Trying to run ["+i+"] = "+cmdarray[i]);
	...
	public static java.util.List<Object> ejsConfirmList(Component _target, Dimension _size, String _message,
			String _title, java.util.List<?> _list) {
		return ejsConfirmList(_target, _size, _message, _title, _list, (JComponent) null);
	}

	/**
	 * This method receives a list of objects which it exposes to the user. The user
	 * can remove objects from the list and click OK. The class then returns the
	 * modified list. If the user clicks cancel it will return null
	 * 
	 * @param _target          Component The dialog will be shown relative to this
	 *                         component
	 ...
	 * @param _bottomComponent JComponent and additional component to show at the
	 *                         bottom
	 * @return AbstractList
	 */
	public static java.util.List<Object> ejsConfirmList(Component _target, Dimension _size, String _message,
			String _title, java.util.List<?> _list, JComponent _bottomComponent) {
		class ReturnValue {
			boolean value = false;
