...
//	}
	 ...
	 * 
	 * Windows: search in \Java typical 64-bit jdk: Program
	 * Files\Java\jdkX.X.X_XX\jre\bin\javaw.exe typical 64-bit jre: Program
	 * Files\Java\jreX.X.X_XX\bin\javaw.exe or Program Files\Java\jreX\bin\javaw.exe
	 * or Program Files\Java\jre-X\bin\javaw.exe typical 32-bit jre: as above, but
	 * in Program Files (x86)\Java\
	 *
	 * OS X: search in: /JavaVirtualMachines typical:
	 * /System/Library/Java/JavaVirtualMachines/X.X.X.jdk/Contents/Home/bin/javaw.exe
	 * ??
	 * 
	 * Linux: search in: /jvm typical: /usr/lib/jvm/java-X-openjdk/jre/bin/javaw.exe
	 * or /usr/lib/jvm/java-X.X.X-openjdk/jre/bin/javaw.exe or
	 * /usr/lib/jvm/java-X-sun-X.X.X.XX/jre/bin/javaw.exe or
	 * /usr/lib/jvm/java-X.X.X-sun/jre/bin/javaw.exe
	 *
	...
	/**
	 * Finds the default JRE of a given bitness (32 or 64). A public JRE is returned
	 * if possible.
	 * 
	 * @param vmBitness the bitness desired
	 ...
	 * 
	 * Win: typical bundled jre: {TRACKER_HOME}\jre\bin\java.exe typical jdk:
	 * Program Files\Java\jdkX.X.X_XX\jre\bin\java.exe typical jre: Program
	 * Files\Java\jreX.X.X_XX\bin\java.exe or Program Files\Java\jreX\bin\java.exe
	 ...
	 * /usr/lib/jvm/java-X-openjdk/jre/bin/java symlink at:
	 * /usr/lib/jvm/java-X.X.X-openjdk/jre/bin/java sun versions: java-X-sun and
	 * java-X.X.X-sun also: /usr/lib/jvm/jre1.8.0_151/bin/java ?
	 *
		...
		if (!isReady) {
			...
			try {
				...
				if (OSPRuntime.isWindows()) {
					...
					String x86 = System.getenv("ProgramFiles(x86)"); //$NON-NLS-1$
					// add Program Files (may or may not be x86) Java directory to search path if it
					// exists
					if (progfiles != null) {
	...
	/**
	 * Finds all JRE directories (recursively) in a directory and adds them to a
	 * Set. Does not search symbolic links.
	 *
	...
	private static class JavaFilter implements FilenameFilter {
		...
		public boolean accept(File dir, String name) {
			if (!dir.getPath().endsWith("bin")) //$NON-NLS-1$
				return false;
			...
			if (dir.getPath().contains("1.5.") || dir.getPath().contains("-5-") //$NON-NLS-1$ //$NON-NLS-2$
				...
				return false;
			if (name.equals("java.exe")) //$NON-NLS-1$
				return true; // windows
