...
public class Launcher {
	...
	/**
	 * Sets the selected tab by name and returns its LaunchPanel, or null if tab not
	 * found. The path passed to this method is one or more node names separated by
	 * /.
	 *
...
//      if(path.startsWith(tab.getRootNode().name)) {
	...
	/**
	 * Sets the selected node by path and returns the node, or null if node not
	 * found. Path is node names separated by / or \
	 *
	...
	/**
	 * Opens an xml document specified by name and displays it in a new tab (or
	 * selects the tab if already open). Name may be a relative path, absolute path,
	 * or self-contained xml string.
	 *
	 * @param name the name
	 * @return the absolute path, or null if failed or currently open
	 */
		...
		if (control.failedToRead()) {
			...
			if (!name.startsWith(defaultFileName) && (jar != null) && !name.startsWith(jar)) {
				...
				JOptionPane.showMessageDialog(null,
						LaunchRes.getString("Dialog.InvalidXML.Message") + " \"" + name + "\"", //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
...
//      }
	...
	protected void refreshStringResources() {
		...
		// lookFeelItem.setText(LaunchRes.getString("Menu.Display.LookAndFeel"));
		// //$NON-NLS-1$
	...
	protected void createGUI(boolean splash) {
		...
		memoryButton.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				for (int next : sizes) {
					if (xsetMemorySize < 9 * next / 10 && (memSize < 9 * next / 10 || memSize > 11 * next / 10)) {
		...
		Component[] comps = new Component[] { Box.createHorizontalGlue(), memoryButton, Box.createHorizontalStrut(2) };
...
//                +"and tabnumber "+selectedNode.tabNumber);  //$NON-NLS-1$
	...
	protected void showAboutDialog() {
		...
		String aboutString = name + " " + vers + newline //$NON-NLS-1$
				+ "Copyright (c) 2025 Wolfgang Christian" + newline //$NON-NLS-1$
	...
	/**
	 * Gets the node path and page number associated with an html URL. May return
	 * null.
	 *
	 * @param html the URL
	...
	/**
	 * Determines whether the specified class is a model. A model class must define
	 * a static getModelPane(String[], Frame) method.
	 *
	...
	protected void handleMousePressed(MouseEvent e, final LaunchPanel tab) {
		...
		if (OSPRuntime.isPopupTrigger(e)) {
			...
			if (isLaunchable(selectedNode)) {
			...
			} else if (selectedNode.getLaunchClass() == null && selectedNode.launchClassName != null
				...
				JOptionPane.showMessageDialog(frame, LaunchRes.getString("Dialog.ClassNotFound.Message1") + //$NON-NLS-1$
						selectedNode.launchClassName + XML.NEW_LINE
						+ LaunchRes.getString("Dialog.ClassNotFound.Message2") + //$NON-NLS-1$
				...
				if (!approved && frames[i].isVisible() && !(frames[i] instanceof LauncherFrame)
					...
					int selected = JOptionPane.showConfirmDialog(frame,
							LaunchRes.getString("Dialog.Terminate.Message") + XML.NEW_LINE //$NON-NLS-1$
									+ LaunchRes.getString("Dialog.Terminate.Question"), //$NON-NLS-1$
							LaunchRes.getString("Dialog.Terminate.Title"), //$NON-NLS-1$
							JOptionPane.YES_NO_OPTION);
					if (selected == JOptionPane.YES_OPTION) {
						approved = true;
					} else {
						...
						if (!approved && !declined) { // ask for approval
							int selected = JOptionPane.showConfirmDialog(frame,
									LaunchRes.getString("Dialog.TerminateSeparateVM.Message") //$NON-NLS-1$
											+ XML.NEW_LINE + LaunchRes.getString("Dialog.TerminateSeparateVM.Question"), //$NON-NLS-1$
									LaunchRes.getString("Dialog.TerminateSeparateVM.Title"), JOptionPane.YES_NO_OPTION);
	...
	public class LauncherFrame extends JFrame {
		...
		private LauncherFrame() {
			...
			try {
				setIconImage(ResourceLoader.getImageIcon(OSPRuntime.OSP_ICON_FILE).getImage());
			} catch (Exception ex) {
				// image not found
			}
		}
...
//		}
	...
	private class LaunchRenderer extends DefaultTreeCellRenderer {
		...
		public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded,
			...
			if ((node.getFileName() != null) && (Launcher.this instanceof LaunchBuilder)) {
				...
				if ((node.getLaunchClass() == null) && (node.launchClassName != null)
						&& !node.launchClassName.equals("")) { //$NON-NLS-1$
			...
			if (launcher.saveState) {
				...
				if (expansions != null) {
					if (rootEnabled) {
						...
						launcher.expansions = expansions;
				}
				if (rootEnabled)
	...
	public static void launch(final Class<?> type, String[] args, final LaunchNode node) {
		...
		// for(int i = 0; i<args.length && args[i]!=null && !"".equals(args[i]); i++) {
		// //$NON-NLS-1$
		// cmd.add(args[i]);
		// }
		// }
	...
	/**
	 * Attempts to relaunch the current jar in a separate VM with specified
	 * arguments and memory size. If successful, the current VM exits.
	 *
	 * @param args       the main method arguments
	...
	private static void findFramesFor(final LaunchNode node, Frame[] prevFrames, final Runnable runner) {
		...
		for (int i = 0; i < frames.length; i++) {
			...
			if (frames[i] instanceof JFrame) {
				...
				if ((frame.getDefaultCloseOperation() == WindowConstants.DO_NOTHING_ON_CLOSE)
						|| (frame instanceof MessageFrame) || (frame instanceof LauncherFrame)) {
					continue;
				}
			}
