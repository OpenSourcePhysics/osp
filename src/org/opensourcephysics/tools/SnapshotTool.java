...
public class SnapshotTool implements Tool {
	 ...
	 * <p>
	 * 
	 * @param filename  the name of a file (the extension indicates the format). If
	 *                  null, teh user will be prompted for a name
	 * @param component the component to get the image from
	 * @param output    An optional output stream to save to. If null the image is
	 *                  saved to a file
	 ...
	 * 
	 * @param filename  the name of a file (the extension indicates the format). If
	 *                  null, the user will be prompted for a name
	 * @param component the component to get the image from
	 * @param output    An optional output stream to save to. If null the image is
	 *                  saved to a file
	 * @param scale     A scale factor that resizes the image. A value of 1 uses the
	 *                  actual size.
	 * @return true if the image was correctly saved
	 */
	public boolean saveImage(String filename, Component component, java.io.OutputStream output, double scale) {
		...
		if ((output == null) && (filename == null)) { // Select target file and scale
			...
			/*
			 * This code is temporarily disabled because it doesn't work properly for all
			 * types of components
			 */
		...
		if (!(supported || "gif".equalsIgnoreCase(format) || "eps".equalsIgnoreCase(format))) { //$NON-NLS-1$ //$NON-NLS-2$
			...
			JOptionPane.showMessageDialog((JFrame) null, message, res.getString("SnapshotTool.Error"), //$NON-NLS-1$
