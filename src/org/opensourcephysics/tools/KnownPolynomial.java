...
public class KnownPolynomial extends PolynomialLeastSquareFit implements KnownFunction {
	...
	public String getParameterDescription(int i) {
		...
		if (getParameterCount() == 2) {
			if (i == 0)
				return ToolsRes.getString("Function.Parameter.Slope.Description"); //$NON-NLS-1$
	...
	public void setParameters(String[] names, double[] values, String[] descriptions) {
		if (names != null) {
			for (int i = 0; i < Math.min(names.length, getParameterCount()); i++) {
				if (names[i] == null || "".equals(names[i].trim())) //$NON-NLS-1$
					continue;
	...
	public String getDescription() {
		if (description != null && !"".equals(description.trim())) //$NON-NLS-1$
			return description;
	...
	public boolean equals(Object f) {
		...
		if (n != poly.getParameterCount())
			return false;
		for (int i = 0; i < n; i++) {
			if (!getParameterName(i).equals(poly.getParameterName(i)))
				return false;
		}
