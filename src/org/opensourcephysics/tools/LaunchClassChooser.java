...
public class LaunchClassChooser extends JDialog {
	...
	private static Map<String, LaunchableClassMap> classMaps = new TreeMap<String, LaunchableClassMap>(); // maps path
																											// to
																											// classMap
	...
	private JList<String> choices; // list of search results (class names)
	...
	/**
	 * Determines if the specified path is loaded. This will return true only if the
	 * path is one or more jar files all of which are loaded.
	 *
	 * @param path the path
	...
	private void search() {
		...
		if (search(searchField.getText())) {
			...
			JOptionPane.showMessageDialog(this,
					LaunchRes.getString("Dialog.InvalidRegex.Message") + " \"" + searchField.getText() + "\"", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					LaunchRes.getString("Dialog.InvalidRegex.Title"), JOptionPane.WARNING_MESSAGE); //$NON-NLS-1$
	...
	/**
	 * Searches for class names using a regular expression string and puts matches
	 * into the class chooser list of choices.
	 *
...
/**
 * A map of class name to launchable Class object. The name of the jar or
 * directory is prepended to the class name.
 */
class LaunchableClassMap extends TreeMap<String, Class<?>> {
	...
	/**
	 * Loads a class from the URLClassLoader or, if this fails, from the current
	 * class loader.
	 *
