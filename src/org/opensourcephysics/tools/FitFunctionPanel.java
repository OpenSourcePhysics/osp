...
public class FitFunctionPanel extends FunctionPanel {
	...
	/**
	 * Constructor with user function editor. The editor must be preloaded with at
	 * least one main function (the fit function);
	 *
			...
			for (int i = 0; i < functions[j].getParameterCount(); i++) {
				if (paramEditor.getObject(functions[j].getParameterName(i)) == null) {
					Parameter param = new Parameter(functions[j].getParameterName(i),
							String.valueOf(functions[j].getParameterValue(i)), functions[j].getParameterDescription(i));
	...
	public void propertyChange(PropertyChangeEvent e) {
		if (e.getPropertyName().equals("edit") && functionTool != null) { //$NON-NLS-1$
			...
			if (functions != null && functions.length > 0) {
				if (e.getSource() == functionEditor && functions[0].getName().equals(e.getOldValue())) {
					// rename this panel
	...
	protected void refreshFunctions() {
		if (paramEditor != null) {
			((UserFunctionEditor) functionEditor).updateAllFunctions(paramEditor);
		}
