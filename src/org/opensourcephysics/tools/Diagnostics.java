...
public class Diagnostics {
	...
	public static void aboutJava() {
		...
		JOptionPane.showMessageDialog(dialogOwner, aboutString, ToolsRes.getString("Diagnostics.Java.About.Title"), //$NON-NLS-1$
...
//      jarFile = new File(codeSource.getLocation().getPath());
	...
	public static void aboutJava3D() {
		...
		if (props != null) {
			...
			String jarPath = j3djar == null ? ToolsRes.getString("Diagnostics.About.Unknown") : //$NON-NLS-1$
					j3djar.getPath();
			...
			JOptionPane.showMessageDialog(dialogOwner, aboutString,
					ToolsRes.getString("Diagnostics.Java3D.About.Title"), //$NON-NLS-1$
					JOptionPane.INFORMATION_MESSAGE);
		} else if (j3djar != null) { // not working but jar exists
			JOptionPane.showMessageDialog(dialogOwner, ToolsRes.getString("Diagnostics.Java3D.Error.Message"), //$NON-NLS-1$
					ToolsRes.getString("Diagnostics.Java3D.About.Title"), //$NON-NLS-1$
					JOptionPane.WARNING_MESSAGE);
		} else { // not working and jar not found
			JOptionPane.showMessageDialog(dialogOwner, ToolsRes.getString("Diagnostics.Java3D.NotFound.Message1") //$NON-NLS-1$
					+ NEWLINE + ToolsRes.getString("Diagnostics.Download.Message") //$NON-NLS-1$
					+ NEWLINE + JAVA_3D_URL, ToolsRes.getString("Diagnostics.Java3D.About.Title"), //$NON-NLS-1$
					JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public static void aboutJOGL() {
		...
		String jarPath = jogljar == null ? ToolsRes.getString("Diagnostics.About.Unknown") : //$NON-NLS-1$
				jogljar.getPath();
		if (version != null) {
			String aboutString = ToolsRes.getString("Diagnostics.JOGL.About.Version") //$NON-NLS-1$
					...
					+ " " + jarPath; //$NON-NLS-1$
			JOptionPane.showMessageDialog(dialogOwner, aboutString, ToolsRes.getString("Diagnostics.JOGL.About.Title"), //$NON-NLS-1$
					JOptionPane.INFORMATION_MESSAGE);
			...
			JOptionPane.showMessageDialog(dialogOwner, ToolsRes.getString("Diagnostics.JOGL.NotFound.Message1") //$NON-NLS-1$
					+ NEWLINE + ToolsRes.getString("Diagnostics.JOGL.NotFound.Message2"), //$NON-NLS-1$
