		...
		if (result == JFileChooser.APPROVE_OPTION) {
			...
			if (file.exists()) {
				int selected = JOptionPane.showConfirmDialog(this,
						ToolsRes.getString("EncryptionTool.Dialog.ReplaceFile.Message") + " " + file.getName() + "?", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	private void createGUI() {
		...
		aboutItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String aboutString = toolname + OSPRuntime.VERSION + XML.NEW_LINE
						+ ToolsRes.getString("EncryptionTool.About.OSPName") + XML.NEW_LINE //$NON-NLS-1$
						+ "www.opensourcephysics.org"; //$NON-NLS-1$
				JOptionPane.showMessageDialog(EncryptionTool.this, aboutString,
