...
//     org.opensourcephysics.tools.DataTool.loadClass = true;
	...
	/**
	 * Autoscale the drawing panel's x axis using min and max values. from
	 * measurable objects.
	 * 
	 * @param autoscale
	 */
	...
	/**
	 * Gets Drawable objects added by the user of an assignable type. The list
	 * contains objects that are assignable from the class or interface.
	 *
	...
	/**
	 * Sets the interactive mouse handler if the drawing panel is an interactive
	 * panel.
	 *
	...
	/**
	 * Sets the interior background color for the current drawing panel. The
	 * interior of a PlottingaPanel is the area inside the axes where is displayed.
	...
	protected void pasteAction(XMLControlElement control) {
		...
		XMLTreeChooser chooser = new XMLTreeChooser(DisplayRes.getString("DrawingFrame.SelectDrawables_chooser_title"), //$NON-NLS-1$
				DisplayRes.getString("DrawingFrame.SelectDrawables_chooser_message"), this); //$NON-NLS-1$
	...
	/**
	 * Adds Views menu items on the menu bar. Override this method to add custom
	 * menu items.
	 */
