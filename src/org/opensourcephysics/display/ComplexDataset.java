...
public class ComplexDataset extends DataTable.DataModel implements Drawable, Measurable, Data {
	...
	/**
	 * Sets the data point marker. Shapes are: AMP_CURVE RE_IM_CURVE PHASE_CURVE
	 * PHASE_BAR PHASE_POST
	 *
	 * @param _markerShape
	 */
	...
	public void append(double x, double re, double im) {
		if (Double.isNaN(x) || Double.isInfinite(x) || Double.isNaN(re) || Double.isInfinite(re) || Double.isNaN(im)
				|| Double.isInfinite(im)) {
			return;
		}
	...
	public void append(double[] _xpoints, double[] _repoints, double[] _impoints) {
		...
		if ((_repoints == null) || (_impoints == null) || (_xpoints.length != _repoints.length)
				|| (_xpoints.length != _impoints.length)) {
		...
		for (int i = 0; i < _xpoints.length; i++) {
			if (Double.isNaN(_xpoints[i]) || Double.isInfinite(_xpoints[i]) || Double.isNaN(_repoints[i])
					|| Double.isInfinite(_repoints[i]) || Double.isNaN(_impoints[i])
					|| Double.isInfinite(_impoints[i])) {
				badData = true;
				continue;
			}
	...
	public void append(double[] _xpoints, double[] _zpoints) {
		...
		for (int i = 0; i < _xpoints.length; i++) {
			if (Double.isNaN(_xpoints[i]) || Double.isInfinite(_xpoints[i]) || Double.isNaN(_zpoints[2 * i])
					|| Double.isInfinite(_zpoints[2 * i]) || Double.isNaN(_zpoints[2 * i + 1])
					|| Double.isInfinite(_zpoints[2 * i + 1])) {
				badData = true;
				continue;
			}
			xmax = Math.max(_xpoints[i], xmax);
	...
	private void cleanBadData() {
		for (int i = 0; i < index; i++) {
			if (Double.isNaN(xpoints[i]) || Double.isInfinite(xpoints[i]) || Double.isNaN(re_points[i])
					|| Double.isInfinite(re_points[i]) || Double.isNaN(im_points[i])
					|| Double.isInfinite(im_points[i])) {
	 ...
	 *
	 * Since data will be partially sorted this should be fast. Added by W.
	 * Christian.
	 */
	...
	private void drawPost(DrawingPanel drawingPanel, Graphics2D g2, double x, double y, int postWidth,
			Color fillColor) {
