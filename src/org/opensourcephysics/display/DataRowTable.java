...
//		}
	...
	/**
	 * Sets the <code>Timer</code>'s initial time delay (in milliseconds) to wait
	 * after the timer is started before firing the first event.
	 * 
	 * @param delay
	 */
	...
	public void setNumericFormat(String str) {
		if ((str != null) && !str.equals(formatPattern)) { // format has changed
			...
			refreshTable("setNumericFormat " + str);
		}
	}
	...
	public void setColumnFormat(int column, String format) {
		DecimalFormat f = org.opensourcephysics.numerics.Util.newDecimalFormat(format);
	...
	/**
	 * Clears data from this table. Column names and format patterns are not
	 * affected.
	 */
...
//	};
  ...
  public TableCellRenderer getCellRendererOrNull(int row, int column, boolean isScrolling) {
	...
	/**
	 * Performs the action for the refresh timer by refreshing the data in the
	 * DataTable.
	 *
	 * @param evt
	 */
