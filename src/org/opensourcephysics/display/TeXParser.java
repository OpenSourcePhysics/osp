...
public class TeXParser {
	 ...
	 * 
	 * @param s the string in TeX-like notation
	 * @return
	 */
	public static String parseTeX(String s) {
		int pt;
		// BH 2020.06.27 no need for regex (String.split) -- just a simple check
		...
		while (pt >= 0) {
			...
			s = s.substring(0, pt) + (ch == null ? s.substring(pt + 2, pt1) : ch) + s.substring(pt1 + off);
			pt = (off == 0 ? -1 : s.indexOf("$\\", pt + 1));
		}
		return s;
//		OSPLog.debug("TeXParser.parse " + s + " " + ++np);
//		String[] chunks = s.split("\\$"); //$NON-NLS-1$
//		// boolean mathMode=(s.charAt(0)=='$');
...
//	static {
//
//		OSPLog.debug(parseTeX("$\\theta$"));
//
//		OSPLog.debug(parseTeX(">>>$\\theta$$\\omega$<<<"));
//
//		OSPLog.debug(parseTeX("$\\theta$"));
//		
//		
...
//
	/**
	 * Removes subscripting notation from an input String. This returns the root
	 * plus appended subscript but without subscripting notation.
	 *
	...
	/**
	 * Removes the subscript from an input String. This returns the root only, with
	 * no subscript.
	 *
