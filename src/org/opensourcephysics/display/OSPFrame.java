...
public class OSPFrame extends JFrame implements Hidable, AppFrame {
	...
	protected ArrayList<JButton> customButtons = new ArrayList<JButton>(); // list of custom buttons, custom buttons are
																			// enabled when the animation is stopped
	static int topx = 10;
	static int topy = 100;

	/**
	 * Set to <I>true</I> if a simulation should automatically render this frame
	 * after every animation step.
	 */
	protected boolean animated = false;

	/**
	 * Set to <I>true</I> if a simulation should automatically clear the data when
	 * it is initialized.
	 */
	protected boolean autoclear = false;

	/**
	 * Set <I>true</I> if the Frame's defaultCloseOperation has been changed by
	 * Launcher.
	 */
   ...
   */
	...
	/**
	 * Sets the title for this frame to the specified string after converting TeX
	 * math symbols to characters.
	 * 
	...
	/**
	 * Adds a child frame that depends on this frame. Child frames are closed when
...
//    	FontSizer.setFonts(getJMenuBar(), level);
	 ...
	 *
	 * The default method does nothing. Override this method to create a menu item
	 * that is appropriate for the frame.
	 */
	 ...
	 *
	 * The default method does nothing. Override this method to select the object(s)
	 * and the data to be cleared.
	 */
	 ...
	 *
	 * The default method does nothing. Override this method to clear and repaint
	 * objects that have data.
	 */
	...
	/**
	 * Removes a menu item with the given name from the menu bar and returns the
	 * removed item. Returns null if menu item does not exist.
	 *
	 * @param menuName String
	 * @return JMenu
	 */
	...
	public JButton addButton(String methodName, String text, String toolTipText, final Object target) {
		...
		try {
			...
			System.err.println(
					"Error adding custom button " + text + ". The method " + methodName + "() does not exist."); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
		customButtons.add(b);
		return b;
	}
	...
	/**
	 * Overrides JFrame method. This converts EXIT_ON_CLOSE to DISPOSE_ON_CLOSE and
	 * sets the wishesToExit flag.
	 *
	...
	/**
	 * Returns true if this frame wishes to exit. Launcher uses this to identify
	 * control frames.
	 *
