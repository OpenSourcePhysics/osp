 ...
 * @version    1.0
 */
public class ComplexInterpolatedPlot2 extends MeasuredImage implements Plot2D {

  GridData griddata;
  //byte[][] rgbData;
  protected byte[] pixelData;
  Grid grid;
  ComplexColorMapper2 colorMap;
   ...
   * @param _griddata
   */
  public ComplexInterpolatedPlot2(GridData _griddata) {
    griddata = _griddata;
    colorMap = new ComplexColorMapper2(1);
    if(griddata==null) {
 ...
 */
  ...
  protected void checkImage(DrawingPanel panel) {
    ...
    OSPLog.finer("ComplexInterpolatedPlot2 image created with width="+width+" and height="+height); //$NON-NLS-1$ //$NON-NLS-2$
...
//    image = new BufferedImage(ccm, raster, false, null);
    ...
    setPixelsOpaque(pixelData);
    update();
  }

  private void setPixelsOpaque(byte[] pixels) {
	...
	protected void writeToRaster(double x0, double y, double dx, double dy) {
		...
		for (int j = 0; j < height; j++) {
			...
			for (int i = 0; i < width; i++) {
				int index = (dy < 0) ? j * width + i : (height - j - 1) * width + i;
				colorMap.samplesToComponents(griddata.interpolate(x, y, indexes, samples), pixelData, index * 4,
						imageType);
				x += dx;
			}
			y += dy;
