...
public class OSPControl extends ControlFrame implements PropertyChangeListener, MainFrame {
	 ...
	 *
	 * Override this method and change the default close operation if this control
	 * is used with an applet.
	 */
	protected void init() {
	...
	/**
	 * Sets the location of the divider between the control table and the message
	 * panel.
	 * 
	 * @param loc int
	 */
	...
	/**
	 * Sets the editable property of the given parameter so that it can not be
	 ...
	 * display until the control is unlocked.
	 *
	 * @param lock boolean
	 */
	@Override
	...
	public void setValue(String par, Object val) {
		...
		if(OSPRuntime.isJS)table.refresh(); // WC: updates table view when value changes
	}
	...
	/**
	 * Gets the object with the specified property name. Throws an
	 * UnsupportedOperationException if the named object has not been stored.
	 *
	 * @param par
	 * @return the object
	 */
	...
	public Collection<String> getPropertyNamesRaw() {
		return table.getPropertyNamesRaw();
	}
	...
	/**
	 * Adds a ControlTableListener that invokes method in the control's model. The
	 * method in the model is invoked with the table's variable name passed as a
	 * parameter.
	 *
	 * @param methodName the name of the method; the method has no parameters
	 */
	...
	/**
	 * Adds a ControlTableListener that invokes method in the given object. The
	 * method in the target is invoked with the table's variable name passed as a
	 * parameter.
	 *
	 * @param methodName the name of the method; the method has no parameters
	...
	/**
	 * A signal that a method has completed. A message is printed in the message
	 * area.
	 *
	 * @param message
	 */
	...
	static class OSPControlLoader implements XML.ObjectLoader {
		...
		protected void saveControlProperites(XMLControl xmlControl, OSPControl ospControl) {
			// save the parameters
			Iterator<String> it = ospControl.getPropertyNamesRaw().iterator();
			while (it.hasNext()) {
				String name = it.next();
	...
	/**
	 * Creates an OSP control and establishes communication between the control and
	 * the model.
	 *
	 * Custom buttons are usually added to this control to invoke actions in the
	 * model.
	 *
	 * @param model Object
