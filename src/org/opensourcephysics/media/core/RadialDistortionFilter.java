...
public class RadialDistortionFilter extends Filter {
	...
	/**
	 * Gets the fixed radius. Pixels at this relative distance from the image center
	 * remain fixed.
	 * 
	...
	public void setSourceProjectionType(String type) {
		if (!sourceProjectionType.equals(type) && PROJECTION_TYPES.contains(type)) {
			...
			hasLowerLimit = getAngleAtRadius(0.5, outputProjectionType, Math.PI / 2) < getAngleAtRadius(0.5,
					sourceProjectionType, Math.PI / 2);
			isValidTransform = false;
			if (inspector != null)
				inspector.updateDisplay();
			firePropertyChange("image", null, null); //$NON-NLS-1$
	...
	/**
	 * Finds the lower radius limit, if any, based on current projection types and
	 * source field of view. The radius limit is the fixed radius below which the
	 * output field of view would exceed 180 degrees. This uses Newton's method to
	 * solve for rLimit as follows: --define f(r) = theta_source and theta_out
	 * assuming current source FOV and 180 degree output FOV --make an initial
	 * estimate of rLimit = 1 (always too big) --find the values of f and df/dr at
	 * the initial estimate --use Newton's method to obtain a better estimate of
	 * rLimit --find new values of f and df/dr --repeat until estimate produces
	 * sufficiently small value of f
	 */
	 ...
	 * 
	 * @param r              the distance from the image center as a fraction of the
	 *                       corner distance
	...
	/**
	 * Gets the derivative dtheta/dr at a given radius. Used to find radius limit
	...
	private class Inspector extends InspectorDlg {
		...
		void createGUI() {
			...
			sourceTypeDropdown.setSelectedItem(
					MediaRes.getString("RadialDistortionFilter.ProjectionType." + sourceProjectionType)); //$NON-NLS-1$
			...
			outputTypeDropdown.setSelectedItem(
					MediaRes.getString("RadialDistortionFilter.ProjectionType." + outputProjectionType)); //$NON-NLS-1$
