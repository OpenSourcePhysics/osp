...
public class ImageVideoRecorder extends ScratchVideoRecorder {
	...
	public String saveVideo(String fileName) throws IOException {
		...
		saveScratch();
		return (savedFilePaths == null || savedFilePaths.length == 0) ? null : savedFilePaths[0];
	}
	...
	// ________________________________ protected methods
...
//  }
	...
	protected boolean append(Image image) {
		...
		try {
			ImageIO.write(source, tempFileType, new BufferedOutputStream(new FileOutputStream(fileName)));
		} catch (Exception e) {
			return false;
		}
