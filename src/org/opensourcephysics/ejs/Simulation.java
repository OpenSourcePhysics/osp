...
public abstract class Simulation implements java.lang.Runnable, StateMachine {
	...
	private int delayJS = (/** @j2sNative delay || */
	(int) delay);
	...
	public void run() {
		...
		stateHelper.sleep(0);
		/*
		 * while(thread==Thread.currentThread()) { step(); try { Thread.sleep(delay); }
	...
	/**
	 * apply user interface changes. Yes, exactly the same as apply() (I need it
	 * somewhere else :-)
	 */
	...
	public void updateAfterModelAction() {
		// if (view!=null) view.initialize(); // If initialize sends all the data, this
		// is redundant
		update();
	}
	 ...
	 * model. If the variable is an array, individual element values are separated
	 * by a comma. Only public variables of primitive or String type can be
	 * accessed.
	 * 
	 ...
	 * This method returns a String with the value of a public variable of the
	 * model. If the variable is an array, individual element values are separated
	 * by the specified separator string. Only public variables of primitive or
	 * String type can be accessed.
	 * 
	 * @param _name The name of a public variable of the model
	...
	/**
	 * This method sets the value of a public variable of the model. If the variable
	 * is an array, individual element values must separated by a comma. In this
	 * case, if the number of values specifies differs with the length of the array
	 * (a warning may be issued and) either the extra values are ignored (if there
	 * are more) or the last elements of the array will be left unmodified (if there
	 * are less). If the values provided cannot be parsed to the variable type (an
	 * error message may be issued and) the method returns false. Only public
	 * variables of primitive or String type can be accessed.
	 * 
	 * @param _name  the name of a public variable of the model
	...
	/**
	 * This method sets the value of a public variable of the model. If the variable
	 * is an array, individual element values must separated by the specified
	 * separator string. In this case, if the number of values specifies differs
	 * with the length of the array (a warning may be issued and) either the extra
	 * values are ignored (if there are more) or the last elements of the array will
	 * be left unmodified (if there are less). Only public variables of primitive or
	 * String type can be accessed. If the values provided cannot be parsed to the
	 * variable type (an error message may be issued and) the method returns false.
	 * 
	 ...
	 * Pairs of the type 'variable=value' must be separated by semicolons. Then they
	 * will be tokenized and sent to setVariable().
	 * 
	...
	/**
	 * This method is used to set more than one variables of the model at once.
	 * Pairs of the type 'variable=value' must be separated by the separator string
	 * _sep. Then they will be tokenized and sent to setVariable(), using _arraySep
	 * as separator string for values of array variables.
	 * 
	 * @param _valueList the string containing the pairs 'variable=value'
	...
	/**
	 * Saves the state of the model either to a file on the disk or to memory. If
	 * the name of the file starts with the prefix "ejs:", then the state of the
	 * model will be saved to memory, otherwise it will be dumped to disk. Security
	 * considerations apply when running the simulation as an applet.
	 * <p>
	 * The state of the model is saved by writing to disk all its public fields
	 * which implement the java.io.Serializable interface. This includes primitives
	 * and arrays.
	 * 
	...
	/**
	 * Reads the state of the model either from a file on the disk, from memory or
	 * from a url location. If the name of the file starts with the prefix "ejs:",
	 * then the state of the model will be read from a memory file that must have
	 * been created previously with the corresponding call to saveState(). If the
	 * name of the file starts with "url:" it will be considered a url location and
	 * the method will attempt to read the file from this url (either locally or
	 * through the network). This file must have been previously created with a call
	 * to saveState() with destination a disk file, and then this file must have
	 * been copied at the right url location. dumped to disk. If the name of the
	 * file does not start with any of those prefixes, then it will be considered to
	 * be a file. Security considerations apply when running the simulation as an
	 * applet.
	 * <p>
	 * The state of the model is read by reading from disk all its public fields
	 * which implement the java.io.Serializable interface. This includes primitives
	 * and arrays.
	 * 
