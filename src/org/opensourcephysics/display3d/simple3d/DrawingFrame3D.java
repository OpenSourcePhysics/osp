...
public class DrawingFrame3D extends OSPFrame
	...
	/**
	 * DrawingFrame constructor specifying the DrawingPanel that will be placed in
	...
	/**
	 * DrawingFrame constructor specifying the title and the DrawingPanel that will
	 * be placed in the center of the content pane.
	 *
	 * @param title
	 * @param _drawingPanel
	 */
	...
	public void setMessage(String msg) {
		((org.opensourcephysics.display3d.simple3d.DrawingPanel3D) drawingPanel).setMessage(msg); // the default message
																									// box
	}
	...
	/**
	 * Adds the drawing panel to the the frame. The panel is added to the center of
	...
	protected void pasteAction() {
		OSPRuntime.paste((s) -> {
			if (s != null) {
				...
				chooser.chooseAsync(control, Drawable.class, () -> {
	 ...
	 *
	 * Override this method to receive notification that data copied to the
	...
	/**
	 * Dummy creator of a LightInspector. To be overwritten by packages that offer
	 * light inspectors
	 * 
	 * @param drawingPanel
	 * @return
	...
	private void createMenuBar() {
					...
					if (printerJob.printDialog()) {
						try {
							...
							JOptionPane.showMessageDialog(DrawingFrame3D.this,
									DisplayRes.getString("DrawingFrame.PrintErrorMessage"), //$NON-NLS-1$
		...
		decorationNoneItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (drawingPanel != null) {
					drawingPanel.getVisualizationHints()
							.setDecorationType(org.opensourcephysics.display3d.core.VisualizationHints.DECORATION_NONE);
					drawingPanel.repaint();
				}
			}
		...
		decorationCubeItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (drawingPanel != null) {
					drawingPanel.getVisualizationHints()
							.setDecorationType(org.opensourcephysics.display3d.core.VisualizationHints.DECORATION_CUBE);
					drawingPanel.repaint();
				}
			}
		...
		decorationAxesItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (drawingPanel != null) {
					drawingPanel.getVisualizationHints()
							.setDecorationType(org.opensourcephysics.display3d.core.VisualizationHints.DECORATION_AXES);
					drawingPanel.repaint();
				}
			}
		...
		cursorNoneItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (drawingPanel != null) {
					drawingPanel.getVisualizationHints()
							.setCursorType(org.opensourcephysics.display3d.core.VisualizationHints.CURSOR_NONE);
					drawingPanel.repaint();
				}
			}
		...
		cursorCubeItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (drawingPanel != null) {
					drawingPanel.getVisualizationHints()
							.setCursorType(org.opensourcephysics.display3d.core.VisualizationHints.CURSOR_CUBE);
					drawingPanel.repaint();
				}
			}
		...
		cursorXYZItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (drawingPanel != null) {
					drawingPanel.getVisualizationHints()
							.setCursorType(org.opensourcephysics.display3d.core.VisualizationHints.CURSOR_XYZ);
					drawingPanel.repaint();
				}
			}
		...
		cursorCrosshairItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (drawingPanel != null) {
					drawingPanel.getVisualizationHints()
							.setCursorType(org.opensourcephysics.display3d.core.VisualizationHints.CURSOR_CROSSHAIR);
					drawingPanel.repaint();
				}
			}
	...
	/**
	 * Gets a menu with the given name from the menu bar. Returns null if menu item
	 * does not exist.
	 *
	...
	/**
	 * Removes a menu with the given name from the menu bar and returns the removed
	 * item. Returns null if menu item does not exist.
	 *
	 * @param menuName String
	 * @return JMenu
	 */
		...
		if (result == JFileChooser.APPROVE_OPTION) {
			...
			if (file.exists()) {
				int selected = JOptionPane.showConfirmDialog(null,
						DisplayRes.getString("DrawingFrame.ReplaceExisting_message") + file.getName() //$NON-NLS-1$
...
/*
 * //CJB private static final GridBagConstraints gbc;
 * 
 * static { gbc = new GridBagConstraints(); gbc.gridx = 0; gbc.gridy = 0;
 * gbc.weightx = 1.0; gbc.weighty = 1.0; gbc.fill = GridBagConstraints.BOTH;
 * gbc.anchor = GridBagConstraints.CENTER; }
 * 
 * 
 * private JPanel wrapInBackgroundImage(JComponent component,Icon
 * backgroundIcon, int verticalAlignment, int horizontalAlignment) {
 * 
 * // make the passed in swing component transparent component.setOpaque(false);
 * 
 * // create wrapper JPanel JPanel backgroundPanel = new JPanel(new
 * GridBagLayout());
 * 
 * // add the passed in swing component first to ensure that it is in front
 * backgroundPanel.add(component, gbc);
 * 
 * // create a label to paint the background image JLabel backgroundImage = new
 ...
 * 
 * // add the background label backgroundPanel.add(backgroundImage, gbc);
 * 
 * return backgroundPanel;
 * 
 * }
 * 
 * public void setBackground(String name){ //Image Transformation (Escale
 * Factor) ImageIcon image = null; image = new
 * ImageIcon(this.getClass().getClassLoader().getResource(name));
 * this.add(wrapInBackgroundImage((JPanel)this.getComponent(),image, JLabel.TOP,
 * JLabel.LEADING)); } //CJB
 */
