...
public class PlotFrame extends DrawingFrame {
	 ...
	 *
	 * Points will be dropped from the beginning of the dataset after the maximum
	...
	public void setMaximumFractionDigits(int maximumFractionDigits) {
		getDataTable().setMaximumFractionDigits(maximumFractionDigits);
	}

	/**
	 * Sets the maximum number of fraction digits to display in all data table
	 * columns with cthe given columnName.
	 *
	 * @param maximumFractionDigits - maximum number of fraction digits to display
	...
	public void setMaximumFractionDigits(String columnName, int maximumFractionDigits) {
		getDataTable().setMaximumFractionDigits(columnName, maximumFractionDigits);
	}
	...
	public void setRowNumberVisible(boolean vis) {
		getDataTable().setRowNumberVisible(vis);
	}
	...
	/**
	 * Clears the data from all datasets. Dataset properties are preserved because
	 * only the data is cleared.
	 */
	@Override
	public void clearData() {
		datasetManager.clear();
		updateTable(DataTable.MODE_CLEAR);
		if (drawingPanel != null) {
	...
	/**
	 * Removes all Datasets and removes all objects from the drawing panel except
	 * the dataset manager. Datasets are removed from the manager and dataset
	 * properties are not preserved.
	 */

	/**
	 * Removes datasets from the manager. New datasets will be created with default
	...
	public synchronized void showDataTable(boolean show) {
		if (show) {
			if (tableFrame == null || !tableFrame.isDisplayable()) {
				tableFrame = new DataTableFrame(getTitle() + " " + DisplayRes.getString("TableFrame.TitleAddOn.Data"), //$NON-NLS-1$ //$NON-NLS-2$
						getDataTable());
				tableFrame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
			}
			updateTable(DataTable.MODE_SHOW);
	...
	public void updateTable(int mode) {
		if ((tableFrame != null) && tableFrame.isShowing()) {
			getDataTable().refreshTable(mode);
		}
	}
