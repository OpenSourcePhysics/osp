...
/**
 * FrameApplet displays a JFrame from an application in an HTML page. This
 * Applet can be used to run applications as applets. Security restrictions may
 * cause some programs to malfunction if the jar file is not signed.
 *
 * @version 0.9 beta
...
public class FrameApplet extends JApplet implements Renderable {
	...
	private void removeWindowListeners(Window frame) {
		...
		for (int i = 0, n = wl.length; i < n; i++) { // remove window listeners because many windows will call exit(0)
														// when closing
